...
public class SysMenuService extends BaseService {
    ...
    public String getTreeJson() {
        List<SysMenu> sysMenus = menuDao.getAll();
        Collections.sort(sysMenus, new PropertyComparator(SysMenu.PROP_DISPLAY_ORDER));
        List<Tree> trees = new ArrayList<Tree>();
        Set<Tree> subtrees = createTree(sysMenus, ComConstants.ROOT_MENU_PARENTID);
     ...
     * @since JDK 1.6
     */
    private Set<Tree> createTree(List<SysMenu> sysMenus, String parentValue) {
        ...
        for (SysMenu menu : sysMenus) {
            ...
            if (parentId == parentValue || (parentId != null && parentId.equals(parentValue))) {
                ...
                node.setAttributes("\"pid\":\"" + parentId + "\"");
                Set<Tree> subLeafs = createTree(sysMenus, id);
                if (subLeafs.size() > 0) {
